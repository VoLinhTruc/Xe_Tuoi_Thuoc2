...
/**
  * @brief  Deializes the SPI peripheral registers to their default reset values.
  * @param  None
  * @retval None
  */
void SPI_DeInit(void)
{
  ((SPI_TypeDef *) SPI_BaseAddress)->CR1    = SPI_CR1_RESET_VALUE;
  ((SPI_TypeDef *) SPI_BaseAddress)->CR2    = SPI_CR2_RESET_VALUE;
  ((SPI_TypeDef *) SPI_BaseAddress)->ICR    = SPI_ICR_RESET_VALUE;
  ((SPI_TypeDef *) SPI_BaseAddress)->SR     = SPI_SR_RESET_VALUE;
  ((SPI_TypeDef *) SPI_BaseAddress)->CRCPR  = SPI_CRCPR_RESET_VALUE;
}
...
void SPI_Init(SPI_FirstBit_TypeDef FirstBit, SPI_BaudRatePrescaler_TypeDef BaudRatePrescaler, SPI_Mode_TypeDef Mode, SPI_ClockPolarity_TypeDef ClockPolarity, SPI_ClockPhase_TypeDef ClockPhase, SPI_DataDirection_TypeDef Data_Direction, SPI_NSS_TypeDef Slave_Management, uint8_t CRCPolynomial)
{
  ...
  ((SPI_TypeDef *) SPI_BaseAddress)->CR1 = (uint8_t)((uint8_t)((uint8_t)FirstBit | BaudRatePrescaler) |
                       (uint8_t)((uint8_t)ClockPolarity | ClockPhase));
  
  /* Data direction configuration: BDM, BDOE and RXONLY bits */
  ((SPI_TypeDef *) SPI_BaseAddress)->CR2 = (uint8_t)((uint8_t)(Data_Direction) | (uint8_t)(Slave_Management));
  
  if (Mode == SPI_MODE_MASTER)
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 |= (uint8_t)SPI_CR2_SSI;
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 &= (uint8_t)~(SPI_CR2_SSI);
  ...
  ((SPI_TypeDef *) SPI_BaseAddress)->CR1 |= (uint8_t)(Mode);
  
  /* CRC configuration */
  ((SPI_TypeDef *) SPI_BaseAddress)->CRCPR = (uint8_t)CRCPolynomial;
}
...
void SPI_Cmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR1 |= SPI_CR1_SPE; /* Enable the SPI peripheral*/
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR1 &= (uint8_t)(~SPI_CR1_SPE); /* Disable the SPI peripheral*/
  }
}
...
void SPI_ITConfig(SPI_IT_TypeDef SPI_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->ICR |= itpos; /* Enable interrupt*/
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->ICR &= (uint8_t)(~itpos); /* Disable interrupt*/
  }
}
...
{
  ((SPI_TypeDef *) SPI_BaseAddress)->DR = Data; /* Write in the DR register the data to be sent*/
}
...
{
  return ((uint8_t)((SPI_TypeDef *) SPI_BaseAddress)->DR); /* Return the data in the DR register*/
}
...
void SPI_NSSInternalSoftwareCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 |= SPI_CR2_SSI; /* Set NSS pin internally by software*/
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 &= (uint8_t)(~SPI_CR2_SSI); /* Reset NSS pin internally by software*/
  }
}
...
{
  ((SPI_TypeDef *) SPI_BaseAddress)->CR2 |= SPI_CR2_CRCNEXT; /* Enable the CRC transmission*/
}
...
void SPI_CalculateCRCCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 |= SPI_CR2_CRCEN; /* Enable the CRC calculation*/
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 &= (uint8_t)(~SPI_CR2_CRCEN); /* Disable the CRC calculation*/
  }
}
...
uint8_t SPI_GetCRC(SPI_CRC_TypeDef SPI_CRC)
{
  ...
  {
    crcreg = ((SPI_TypeDef *) SPI_BaseAddress)->TXCRCR;  /* Get the Tx CRC register*/
  }
  else
  {
    crcreg = ((SPI_TypeDef *) SPI_BaseAddress)->RXCRCR; /* Get the Rx CRC register*/
  }
...
{
  return ((SPI_TypeDef *) SPI_BaseAddress)->CRCPR; /* Return the CRC polynomial register */
}
...
void SPI_BiDirectionalLineConfig(SPI_Direction_TypeDef SPI_Direction)
{
  ...
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 |= SPI_CR2_BDOE; /* Set the Tx only mode*/
  }
  else
  {
    ((SPI_TypeDef *) SPI_BaseAddress)->CR2 &= (uint8_t)(~SPI_CR2_BDOE); /* Set the Rx only mode*/
  }
}
...
FlagStatus SPI_GetFlagStatus(SPI_Flag_TypeDef SPI_FLAG)
{
  ...
  if ((((SPI_TypeDef *) SPI_BaseAddress)->SR & (uint8_t)SPI_FLAG) != (uint8_t)RESET)
  {
...
void SPI_ClearFlag(SPI_Flag_TypeDef SPI_FLAG)
{
  ...
  /* Clear the flag bit */
  ((SPI_TypeDef *) SPI_BaseAddress)->SR = (uint8_t)(~SPI_FLAG);
}
...
ITStatus SPI_GetITStatus(SPI_IT_TypeDef SPI_IT)
{
  ...
  enablestatus = (uint8_t)((uint8_t)((SPI_TypeDef *) SPI_BaseAddress)->SR & itmask2);
  /* Check the status of the specified SPI interrupt */
  if (((((SPI_TypeDef *) SPI_BaseAddress)->ICR & itpos) != RESET) && enablestatus)
  {
...
void SPI_ClearITPendingBit(SPI_IT_TypeDef SPI_IT)
{
  ...
  ((SPI_TypeDef *) SPI_BaseAddress)->SR = (uint8_t)(~itpos);
